...
namespace LanternKeeper.Behaviours
{
    public class PoisonDagger : PhysicsProp
    {
        ...
        public override void ItemActivate(bool used, bool buttonDown = true)
        {
            ...
            RoundManager.PlayRandomClip(daggerAudio, swingSFX);
        ...
        public void HitDagger(bool cancel = false)
        {
            ...
            if (!cancel && Time.realtimeSinceStartup - timeAtLastDamageDealt > 0.43f)
            {
                ...
                foreach (RaycastHit daggerHit in objectsHitByDaggerList)
                {
                    ...
                    {
                        if (!daggerHit.transform.TryGetComponent<IHittable>(out var component) || daggerHit.transform == previousPlayerHeldBy.transform)
                            continue;
                        if (!(daggerHit.point == Vector3.zero) && Physics.Linecast(previousPlayerHeldBy.gameplayCamera.transform.position, daggerHit.point, out RaycastHit hitInfo, StartOfRound.Instance.collidersAndRoomMaskAndDefault))
                            continue;
                        hitDetected = true;
                        try
                        {
                            if (Time.realtimeSinceStartup - timeAtLastDamageDealt > 0.43f)
                            {
                                ...
                                component.Hit(daggerHitForce, previousPlayerHeldBy.gameplayCamera.transform.forward, previousPlayerHeldBy, playHitSFX: true, 5);
        ...
        public IEnumerator PoisonEnemyCoroutine(EnemyAI enemy)
        {
            ...
            while (timePassed < ConfigManager.daggerPoisonDuration.Value)
            {
                if (Mathf.FloorToInt(timePassed * 10) % 10 == 0)
                    enemy.SetEnemyStunned(setToStunned: true, ConfigManager.daggerPoisonStunDuration.Value, playerHeldBy);
